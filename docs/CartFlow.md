...
3. **Checkout**
  ...
  - Subscription creation now performs exponential backoff with jitter whenever Authorize.Net responds with `E00040` (“record cannot be found”). Up to five retries run within 60 seconds while the checkout UI switches to a “We’re still processing! Please keep this window open.” banner so the member knows to wait. If the profile is still unavailable after those retries, the plugin schedules a single-use cron event 30 minutes later and marks the member as active immediately so benefits apply without delay.
